...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  // hypothesis: -3m/s^2 deceleration
  const double deceleration = -3;
  const auto look_ahead_distance =
      -velocity_mag * velocity_mag / deceleration +
      0.5 * velocity_mag * velocity_mag / deceleration;  // <- Fix This

  // LOG(INFO) << "Calculated look_ahead_distance: "
  // << look_ahead_distance;
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.velocity.x =
          _speed_limit * std::cos(goal.rotation.yaw);  // <- Fix This
      goal.velocity.y =
          _speed_limit * std::sin(goal.rotation.yaw);  // <- Fix This
      goal.velocity.z = 0;
    }
